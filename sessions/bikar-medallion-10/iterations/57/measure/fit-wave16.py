#!/usr/bin/env python3
"""Iter-58 wave-16 consensus + model fit + grid snap — TWO free
(asymmetric) polygon families, mirror-pair wave (wave-7 topology at the
star-tip axes).

Question this answers (Tenet 9): which exact (r, rel) vertices reproduce
the wave-16 sail/fin tiles so FOUR rotated cycles (inner pair + outer
pair, each one cycle + its mirror, wave-7/10/11/14 style) stamp all 40
tiles? Plan thetas cluster at rel ~ +-2.7 (inner family, 20 members)
and +-6.7 (outer family, 20 members) about the STAR-TIP axes; the zoom
witnesses (ids 392/193) are curved sail silhouettes with a thin needle
tail, NOT mirror-symmetric tiles — each family is fitted as a free
polygon via folded consensus (rel<0 members mirrored into the + frame).
...
Pipeline per family (consensus and fit share one grid):
  ...
  4. snap probe g=0.5 (M=720) FIRST; escalate to g=0.25 (M=1440) only
     if the g=0.5 loss > 0.01 IoU. These are the smallest tiles yet
     (~85 u^2 < w11's 135) — expect escalation per the smallness driver,
     but probe both before accepting.

Outputs: wave16-fit.json (both families), wave16-fit-overlay-{inner,outer}.png
"""
...
OUT = SESS / "iterations/57/measure"
...
w16 = next(w for w in plan["waves"] if w["wave"] == 16)
...
# inner family y ~ 2-13 (rel 0.7-3.6 deg at r 178-195); outer y ~ 16-37
# (rel 4.9-10.1 deg at r 195-209).
GRID = 0.25
X0, X1, Y0, Y1 = 170.0, 215.0, 1.0, 40.0
...
def fit_family(name: str, members: list) -> dict:
    ...
    Image.fromarray(np.flipud(rgb)).resize((NX * 3, NY * 3), Image.NEAREST).save(
        OUT / f"wave16-fit-overlay-{name}.png")
...
def fam_of(s):
    th_axis = round(s["theta_deg"] / 36) * 36
    return "inner" if abs(s["theta_deg"] - th_axis) < 4.5 else "outer"
...
(OUT / "wave16-fit.json").write_text(json.dumps(result, indent=1))
print("\nwave16-fit.json written")

"""Iter-58 wave-16 measurement — segment the reference exactly as plan-waves
does, pull the wave-16 shape masks, and extract canonical-frame vertex
...
to the 36-deg STAR-TIP axis (0 mod 36), reproduces the 40 deep_navy
wave-16 tiles (~85 u^2, outer edge)? Plan thetas cluster at rel ~ +-2.7
and +-6.6 about the star-tip axes — TWO mirror-pairs per axis (wave-7
topology, star-tip variant): inner pair at +-2.7, outer pair at +-6.6
(or two distinct shapes — check the zoom witnesses). Tiny tiles
(~85 u^2 < w11's 135): expect the g=0.25 snap escalation, probe g=0.5
first per protocol.

Outputs under iterations/57/measure/:
    wave16-tinted.png    reference crop with wave-16 shapes tinted red
    wave16-zoom-<id>.png mask + image zoom of four sample shapes
    wave16-vertices.json canonical-frame vertex clusters + per-shape areas
"""
...
Image.fromarray(tinted[y0:y1, x0:x1]).save(OUT / "wave16-tinted.png")
...
for s in sorted(real, key=lambda s: abs(s['theta_deg'] % 36 - 18))[:4]:
    ...
    Image.fromarray(big).save(OUT / f"wave16-zoom-{sid}.png")
...
(OUT / "wave16-vertices.json").write_text(json.dumps({

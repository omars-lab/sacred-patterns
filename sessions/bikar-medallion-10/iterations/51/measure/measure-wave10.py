#!/usr/bin/env python3
"""Iter-50 wave-10 measurement — segment the reference exactly as plan-waves
does, pull the wave-10 shape masks, and extract canonical-frame vertex
...
the 36-deg dart axis, reproduces the 10 on-axis navy wave-10 tiles?
...
Outputs under iterations/51/measure/:
    wave10-tinted.png    reference crop with wave-10 shapes tinted red
    wave10-zoom-<id>.png mask + image zoom of two sample shapes
    wave10-vertices.json canonical-frame vertex clusters + per-shape areas
"""
...
w8 = next(w for w in plan["waves"] if w["wave"] == 10)
...
Image.fromarray(tinted[y0:y1, x0:x1]).save(OUT / "wave10-tinted.png")
...
for s in real[:2]:
    ...
    Image.fromarray(big).save(OUT / f"wave10-zoom-{sid}.png")
...
(OUT / "wave10-vertices.json").write_text(json.dumps({

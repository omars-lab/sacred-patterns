#!/usr/bin/env python3
"""Iter-52 wave-10 consensus + closing probe + contour read.

Question this answers (Tenet 9): what polygon at dart-axis rel ~ +6 deg
reproduces the wave-10 deep-navy tile (20 witnesses in mirror pairs at
+-6.1 deg, ~83.7u^2, r ~ 128-144)? All witnesses fold to the + side
(rel < 0 shapes mirror about the 18-deg dart axis), so the consensus
models ONE pair member; the .bkr stamps it plus its mirror, wave-7
style. The closing probe decides whether contour notches are real
boundary (wave-9 finding) or band-chamfer noise (wave-8 finding).

Outputs: wave10-consensus-overlay.png, wave10-contour.json
"""
...
for gx, gy in approx:
    x = X0 + (gx + 0.5) * GRID
    y = Y0 + (gy + 0.5) * GRID
    verts.append((math.hypot(x, y), math.degrees(math.atan2(y, x)) - 18.0))
print("\nraw consensus contour (eps 2.0):")
for r, rel in verts:
    print(f"  r {r:7.2f}  rel {rel:7.2f}")
(OUT / "wave10-contour.json").write_text(json.dumps(
...
Image.fromarray(np.flipud(rgb)).resize((NX * 4, NY * 4), Image.NEAREST).save(
    OUT / "wave10-consensus-overlay.png")

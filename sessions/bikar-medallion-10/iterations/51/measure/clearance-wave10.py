#!/usr/bin/env python3
"""Iter-52 wave-10 clearance proof — rasterize the 20 snapped teardrops
(10 dart axes x {+, mirror}) in reference px space; count overlap px
against reference waves 1-9 (must be ~0) and wave-10 (on-target)."""
...
fit = json.loads((SESS / "iterations/51/measure/wave10-fit.json").read_text())
...
for wnum in range(1, 11):
    ...
    ov = (stamp & wmask).sum()
    tag = "ON-TARGET" if wnum == 10 else ("OK" if ov == 0 else "OVERLAP!")

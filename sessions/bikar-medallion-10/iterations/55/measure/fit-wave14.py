#!/usr/bin/env python3
"""Iter-56 wave-14 consensus + model fit + grid snap — free (asymmetric)
polygon, mirror-pair wave.

Question this answers (Tenet 9): which exact (r, rel) vertices reproduce
the wave-14 royal shield tile at star-tip-axis rel ~ +9.5 so TWO rotated
cycles (one + its mirror, wave-7/10/11 style) stamp all 20 tiles? The
tile is a pair member, NOT self-symmetric (zoom witnesses 32/37 are
mirror images), so the fit is unconstrained; the mirror partner reuses
the same circles with mirrored cpt indices.

Pipeline (consensus and fit share one grid):
  1. folded consensus — all 20 members mapped to the + frame (the
     rel<0 members mirrored), area-matched threshold at TARGET_AREA;
  2. closing probe at 3/4u — small fill = concavities are real
     boundary, model the raw consensus (waves 9-13 precedent);
  3. approxPolyDP eps sweep -> IoU coordinate descent -> DOF-penalized
     pick (IoU - 0.004 * n_verts);
  4. snap probe g=0.5 (M=720, cpt 2x / -2x mod 720) FIRST; escalate to
     g=0.25 (M=1440, cpt 4x / -4x mod 1440) only if the g=0.5 loss
     > 0.01 IoU (w13 finding: the discriminator is angular feature
     scale — these tiles are wide (~10 deg span at r~177), so g=0.5
     may hold, but probe both before accepting).

Outputs: wave14-fit.json, wave14-fit-overlay.png
"""
...
OUT = SESS / "iterations/55/measure"
...
w14 = next(w for w in plan["waves"] if w["wave"] == 14)
...
real = [s for s in w14["shapes"] if not s["fragment"]]

GRID = 0.25
X0, X1, Y0, Y1 = 155.0, 200.0, 5.0, 55.0
...
results = {}
for eps in (2.0, 2.5, 3.0, 3.5, 4.0, 5.0, 6.0):
...
# cpt on the star-tip axis (0 deg): + member vertex at rel x -> cpt(x/g
# mod M); mirror partner cpt(-x/g mod M). Exact when x is on the grid.
rows = []
for r, x in snapped:
    kp = round(x / g) % M
    km = round(-x / g) % M
...
(OUT / "wave14-fit.json").write_text(json.dumps(result, indent=1))

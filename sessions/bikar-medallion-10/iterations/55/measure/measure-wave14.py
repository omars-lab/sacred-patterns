"""Iter-56 wave-14 measurement — segment the reference exactly as plan-waves
does, pull the wave-14 shape masks, and extract canonical-frame vertex
...
to the 36-deg STAR-TIP axis (0 mod 36), reproduces the 20 royal wave-14
tiles (~587 u^2, r_frac ~0.628, outer edge)? Plan thetas straddle the
star-tip axes at rel ~ +-9.5 deg — MIRROR PAIRS like wave 11, so the
expectation is one asymmetric tile model + its mirror, NOT an
axis-symmetric single (th_axis = round(theta/36)*36; the +- rel sign
keys which member).

Outputs under iterations/55/measure/:
    wave14-tinted.png    reference crop with wave-14 shapes tinted red
    wave14-zoom-<id>.png mask + image zoom of two sample shapes
    wave14-vertices.json canonical-frame vertex clusters + per-shape areas
"""
...
Image.fromarray(tinted[y0:y1, x0:x1]).save(OUT / "wave14-tinted.png")
...
for s in real[:2]:
    ...
    Image.fromarray(big).save(OUT / f"wave14-zoom-{sid}.png")
...
(OUT / "wave14-vertices.json").write_text(json.dumps({

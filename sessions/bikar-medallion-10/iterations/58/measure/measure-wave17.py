"""Iter-59 wave-17 measurement — segment the reference exactly as plan-waves
does, pull the wave-17 shape masks, and extract canonical-frame vertex
...
to the 36-deg DART axis (18 mod 36), reproduces the 20 navy wave-17
tiles (~595 u^2, outer edge, r_frac 0.723-0.733)? Plan thetas cluster
at rel ~ +-4.6 about the DART axes — ONE mirror pair per axis (wave-10
topology at the outer edge). Large tiles (~595 u^2 > w11's 135): g=0.5
may hold, but probe both grids per protocol — the discriminator is
angular feature scale, not area.

Outputs under iterations/58/measure/:
    wave17-tinted.png    reference crop with wave-17 shapes tinted red
    wave17-zoom-<id>.png mask + image zoom of four sample shapes
    wave17-vertices.json canonical-frame vertex clusters + per-shape areas
"""
...
OUT = SESS / "iterations/58/measure"
...
w17 = next(w for w in plan["waves"] if w["wave"] == 17)
...
Image.fromarray(tinted[y0:y1, x0:x1]).save(OUT / "wave17-tinted.png")
...
for s in sorted(real, key=lambda s: abs(s['theta_deg'] % 36 - 18))[:4]:
    ...
    Image.fromarray(big).save(OUT / f"wave17-zoom-{sid}.png")
    print(f"zoom id {sid}: theta {s['theta_deg']} r_frac {s['r_frac']} area {s['area_px']}")

# --- canonical-frame vertex extraction: rotate each repeat's DART axis to 0.
...
(OUT / "wave17-vertices.json").write_text(json.dumps({

#!/usr/bin/env python3
"""Iter-59 wave-17 consensus + model fit + grid snap — ONE free
(asymmetric) polygon family, mirror-pair wave (wave-10 topology at the
DART axes, outer edge).

Question this answers (Tenet 9): which exact (r, rel) vertices reproduce
the wave-17 navy pentagon tiles so TWO rotated cycles (one + member,
one mirror, wave-10/14 style) stamp all 20 tiles? Plan thetas cluster
at rel ~ +-4.6 about the DART axes; the zoom witnesses (ids 149/288)
are large house/kite PENTAGONS (~595 u^2) with a short pointed tail —
...
Pipeline (consensus and fit share one grid):
  ...
  4. snap probe g=0.5 (M=720) FIRST; escalate to g=0.25 (M=1440) only
     if the g=0.5 loss > 0.01 IoU. Large tile (~595 u^2 >> w11's 135)
     with no needle features — g=0.5 may hold, but probe both: the
     discriminator is angular feature scale, not area.

Outputs: wave17-fit.json, wave17-fit-overlay-pent.png
"""
...
# Measured folded extents (wave17-vertices.json): x 186.7-224.4,
# y -1.3-39.4 (the y<0 sliver is hairline noise crossing the axis).
GRID = 0.25
X0, X1, Y0, Y1 = 182.0, 230.0, -4.0, 43.0
...
def fit_family(name: str, members: list) -> dict:
    ...
    Image.fromarray(np.flipud(rgb)).resize((NX * 3, NY * 3), Image.NEAREST).save(
        OUT / f"wave17-fit-overlay-{name}.png")
...
(OUT / "wave17-fit.json").write_text(json.dumps(result, indent=1))
print("\nwave17-fit.json written")

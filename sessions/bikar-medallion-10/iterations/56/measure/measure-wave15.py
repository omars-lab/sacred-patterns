"""Iter-57 wave-15 measurement — segment the reference exactly as plan-waves
does, pull the wave-15 shape masks, and extract canonical-frame vertex
...
to the 36-deg DART axis (18 mod 36), reproduces the 10 deep_navy wave-15
tiles (~181 u^2, r_frac ~0.648, outer edge, r 182-185)? Plan thetas sit
ON the dart axes (rel jitter +-0.5 = registration noise) — one
axis-symmetric tile per axis like wave 12, NOT a mirror pair
(th_axis = (theta//36)*36 + 18). Small tiles (~181 u^2, w11-scale):
the snap probe may need the g=0.25 escalation.

Outputs under iterations/56/measure/:
    wave15-tinted.png    reference crop with wave-15 shapes tinted red
    wave15-zoom-<id>.png mask + image zoom of two sample shapes
    wave15-vertices.json canonical-frame vertex clusters + per-shape areas
"""
...
Image.fromarray(tinted[y0:y1, x0:x1]).save(OUT / "wave15-tinted.png")
...
for s in real[:2]:
    ...
    Image.fromarray(big).save(OUT / f"wave15-zoom-{sid}.png")
...
(OUT / "wave15-vertices.json").write_text(json.dumps({

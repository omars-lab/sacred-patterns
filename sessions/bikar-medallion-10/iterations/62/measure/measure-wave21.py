"""Iter-62 wave-21 measurement — segment the reference exactly as plan-waves
does, pull the wave-21 shape masks, and extract canonical-frame vertex
...
relative to the 36-deg STAR-TIP axis (0 mod 36), reproduce the 39 real
small deep_navy wave-21 tiles (~86 u^2, r_frac 0.820-0.863, +4
fragments)? Plan thetas cluster at rel ~ +-2..6 about the star-tip
axes (e.g. 2.4/6.0 above 0, 30.6/34.2 below 36) — that reads as TWO
mirror pairs per axis (4 tiles x 10 = 40; one member missing or
fragmented, hence 39). Expect possibly TWO families (inner-rel ~2 and
outer-rel ~6, w16 inner/outer precedent) — fold and cluster by |rel|
before fitting. Pre-stated witness expectation (Tenet 26): small
compact 3-5-vert tiles (triangle/dart-like), ~86 u^2, in the
outermost lobe band between the wave-20 dart-axis pentagons. Small
tile: probe both grids per protocol — the smallness driver alone
predicts escalation to g=0.25 (w15/w16/w19 precedent).

Outputs under iterations/62/measure/:
    wave21-tinted.png    reference crop with wave-21 shapes tinted red
    wave21-zoom-<id>.png mask + image zoom of four sample shapes
    wave21-vertices.json canonical-frame vertex clusters + per-shape areas
"""
...
w18 = next(w for w in plan["waves"] if w["wave"] == 21)
...
Image.fromarray(tinted[y0:y1, x0:x1]).save(OUT / "wave21-tinted.png")
...
for s in sorted(real, key=lambda s: abs((s['theta_deg'] + 18) % 36 - 18))[:4]:
    ...
    Image.fromarray(big).save(OUT / f"wave21-zoom-{sid}.png")
...
(OUT / "wave21-vertices.json").write_text(json.dumps({

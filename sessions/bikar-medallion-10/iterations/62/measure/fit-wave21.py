"""Iter-62 wave-21 consensus + model fit + grid snap — TWO free
(asymmetric) polygon families, mirror-pair wave (wave-16 two-family
topology at the STAR-TIP axes, outermost band).

Question this answers (Tenet 9): which exact (r, rel) vertices
reproduce the wave-21 small deep_navy wedge tiles so FOUR rotated
cycles (inner pair + outer pair, each one cycle + its mirror,
wave-16 style) stamp all 40 positions (39 real + 1 fragmented in the
plan)? Plan rels cluster at ~ +-2.05 (inner family, 20 members) and
~ +-5.7 (outer family, 19 members) about the star-tip axes; the zoom
witnesses (ids 86/76 inner, 12 outer) are small compact wedge/sail
silhouettes (~86 u^2) wedged between strapwork bands — matching the
pre-stated "small triangle/dart-like" expectation (Tenet 26); thin
spurs in the masks are grow-back noise the area-matched consensus
washes out. Each family is fitted as a free polygon via folded
consensus (rel<0 members mirrored into the + frame).
...
Pipeline per family (consensus and fit share one grid):
  ...
  4. snap probe g=0.5 (M=720) FIRST; escalate to g=0.25 (M=1440) only
     if the g=0.5 loss > 0.01 IoU. Same smallness class as w15/w16/w19
     (~86 u^2) — expect escalation per the smallness driver, but probe
     both before accepting.

Outputs: wave21-fit.json (both families), wave21-fit-overlay-{inner,outer}.png
"""
...
OUT = SESS / "iterations/62/measure"
...
w16 = next(w for w in plan["waves"] if w["wave"] == 21)
...
real = [s for s in w16["shapes"] if not s["fragment"]]

# Window is Cartesian in units: X radial-ish, Y TANGENTIAL u (not deg).
# Measured folded extents (wave21-vertices.json): inner x 233.5-254.8,
# y 0.6-16.1; outer x 222.8-240.2, y 14.1-33.2.
# (rel 4.9-10.1 deg at r 195-209).
GRID = 0.25
X0, X1, Y0, Y1 = 218.0, 260.0, 0.0, 38.0
...
def fit_family(name: str, members: list) -> dict:
    ...
    Image.fromarray(np.flipud(rgb)).resize((NX * 3, NY * 3), Image.NEAREST).save(
        OUT / f"wave21-fit-overlay-{name}.png")
...
def fam_of(s):
    th_axis = round(s["theta_deg"] / 36) * 36
    return "inner" if abs(s["theta_deg"] - th_axis) < 4.0 else "outer"


result = {
    name: fit_family(name, [s for s in real if fam_of(s) == name])
    for name in ("inner", "outer")
}
(OUT / "wave21-fit.json").write_text(json.dumps(result, indent=1))
print("\nwave21-fit.json written")

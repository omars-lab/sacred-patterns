#!/usr/bin/env python3
"""Iter-55 wave-13 measurement — segment the reference exactly as plan-waves
does, pull the wave-13 shape masks, and extract canonical-frame vertex
...
to the 36-deg STAR-TIP axis (0 mod 36), reproduces the 10 turquoise
wave-13 tiles (~591 u^2, r_frac ~0.586, ONE per axis at thetas
0.4/36.4/72.2/...)? Plan thetas sit ON the star-tip axes (rel -0.1..0.4),
so the expectation is an axis-symmetric tile like waves 8/9/12 —
th_axis = round(theta/36)*36.

Outputs under iterations/54/measure/:
    wave13-tinted.png    reference crop with wave-13 shapes tinted red
    wave13-zoom-<id>.png mask + image zoom of two sample shapes
    wave13-vertices.json canonical-frame vertex clusters + per-shape areas
"""
...
OUT = SESS / "iterations/54/measure"
...
w13 = next(w for w in plan["waves"] if w["wave"] == 13)
...
real = [s for s in w13["shapes"] if not s["fragment"]]
...
Image.fromarray(tinted[y0:y1, x0:x1]).save(OUT / "wave13-tinted.png")
...
for s in real[:2]:
    ...
    Image.fromarray(big).save(OUT / f"wave13-zoom-{sid}.png")
...
(OUT / "wave13-vertices.json").write_text(json.dumps({

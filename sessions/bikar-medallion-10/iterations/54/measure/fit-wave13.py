#!/usr/bin/env python3
"""Iter-55 wave-13 consensus + model fit + grid snap — AXIS-SYMMETRIC.

Question this answers (Tenet 9): which exact (r, x) rows reproduce the
wave-13 turquoise teardrop/leaf tile so ONE rotated cycle per STAR-TIP
axis (0 mod 36) stamps all 10? Both zoom witnesses (ids 48/49) show an
axis-symmetric tile — rounded-pentagon body, apex pointing outward — so
the model is the wave-12 symmetric-rows parametrization: on-axis inner
vert + ordered (r, +-x) pairs walking base -> apex + on-axis apex vert.

Pipeline in one script (consensus and fit share the same grid):
  ...
  4. snap probe g=0.5 (M=720) first, g=0.25 (M=1440) fallback only if
     the g=0.5 loss > 0.01 IoU (wave-12 precedent: big tiles hold).
...
Outputs: wave13-fit.json, wave13-fit-overlay.png
"""
...
GRID = 0.25
X0, X1, Y0, Y1 = 140.0, 195.0, -25.0, 25.0
...
(OUT / "wave13-fit.json").write_text(json.dumps(result, indent=1))

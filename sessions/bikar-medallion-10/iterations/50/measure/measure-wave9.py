#!/usr/bin/env python3
"""Iter-50 wave-9 measurement — segment the reference exactly as plan-waves
does, pull the wave-9 shape masks, and extract canonical-frame vertex
...
the 36-deg dart axis, reproduces the 10 star-tip-axis cyan wave-9 tiles?
...
Outputs under iterations/50/measure/:
    wave9-tinted.png    reference crop with wave-9 shapes tinted red
    wave9-zoom-<id>.png mask + image zoom of two sample shapes
    wave9-vertices.json canonical-frame vertex clusters + per-shape areas
"""
...
w9mask = np.isin(labels, ids)
tinted = dim.copy()
tinted[w9mask] = [255, 40, 40]
ys, xs = np.nonzero(medallion)
y0, y1, x0, x1 = ys.min(), ys.max(), xs.min(), xs.max()
Image.fromarray(tinted[y0:y1, x0:x1]).save(OUT / "wave9-tinted.png")
...
for s in real[:2]:
    ...
    Image.fromarray(big).save(OUT / f"wave9-zoom-{sid}.png")
...
for s in real:
    m = labels == s["id"]
    th_axis = round(s["theta_deg"] / 36) * 36  # star-tip axis (0 mod 36), folded to 18 in canon
...
(OUT / "wave9-vertices.json").write_text(json.dumps({

#!/usr/bin/env python3
"""Iter-50/51 wave-9 consensus + contour model — the cyan star-tip-axis
interstice blob (10 witnesses, ~872 u^2, spiky outline cut by band
crossings).

Question this answers (Tenet 9): what polygon, in canonical star-tip
frame (axis at 18 deg), reproduces the wave-9 cyan region — and with how
many vertices? The shape is too spiky for a low-DOF parametric model, so
the model IS the consensus contour: build the 20-witness area-matched
consensus, extract its simplified outline (Douglas-Peucker), symmetrize
about the axis, and report (r, rel) vertices for grid snapping.

Outputs: wave9-consensus-overlay.png (consensus + extracted polygon)
         wave9-contour.json ((r, rel) vertices, areas, IoU of the model)
"""
...
TARGET_AREA = 1485.8 / PXU**2  # ~872.6 u^2
CELL = GRID * GRID
...
for _ in range(40):
    ...
    if (acc >= mid).sum() * CELL > TARGET_AREA:
        ...
        hi = mid
cons = acc >= (lo + hi) / 2
# keep only the largest connected component (stray witness speckle off-window)
...
# --- extract the consensus's own simplified contour --------------------------
import cv2
cnts, _ = cv2.findContours(cons.astype(np.uint8), cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_NONE)
c = max(cnts, key=cv2.contourArea)
for eps_px in (2.0, 2.5, 3.0):
    approx = cv2.approxPolyDP(c, eps_px, True)[:, 0, :]
    print(f"eps {eps_px}: {len(approx)} vertices")
approx = cv2.approxPolyDP(c, 2.5, True)[:, 0, :].astype(float)

# grid coords -> canonical (x, y) -> (r, rel); symmetrize about rel 0
verts = []
for gx, gy in approx:
    x = X0 + (gx + 0.5) * GRID
    y = Y0 + (gy + 0.5) * GRID
...
# order CCW starting from most-negative rel; report raw, snapping comes next
gxc = X0 + (np.arange(NX) + 0.5) * GRID
gyc = Y0 + (np.arange(NY) + 0.5) * GRID
GXC, GYC = np.meshgrid(gxc, gyc)

def poly_mask(vrt):
    pts = [(r * math.cos(math.radians(18.0 + rel)), r * math.sin(math.radians(18.0 + rel))) for r, rel in vrt]
...
pm = poly_mask(verts)
iou = float((cons & pm).sum() / (cons | pm).sum())
print(f"contour model: {len(verts)} verts, IoU vs consensus {iou:.3f}")
for r, rel in verts:
    print(f"  r {r:7.2f}  rel {rel:7.2f}")

(OUT / "wave9-contour.json").write_text(json.dumps(
    {"iou": iou, "verts": [{"r": round(r, 2), "rel": round(rel, 2)} for r, rel in verts]}, indent=1))

rgb = np.zeros((NY, NX, 3), dtype=np.uint8)
rgb[cons] = [150, 150, 150]
edge = pm & ~ndimage.binary_erosion(pm)
rgb[edge] = [255, 40, 40]
# axis line rel=0 for orientation
...
Image.fromarray(np.flipud(rgb)).resize((NX * 3, NY * 3), Image.NEAREST).save(
    OUT / "wave9-consensus-overlay.png")

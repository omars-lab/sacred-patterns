#!/usr/bin/env python3
"""Iter-54 wave-12 measurement — segment the reference exactly as plan-waves
does, pull the wave-12 shape masks, and extract canonical-frame vertex
...
the 36-deg DART axis (18 mod 36), reproduces the 10 navy wave-12 tiles
(~644 u^2, r_frac ~0.548, ONE per axis at thetas 18.5/54.3/90.0/...)?
Plan thetas sit ON the dart axes (rel ~ +-0.5), so the expectation is an
axis-symmetric tile like waves 8/9 — th_axis = (theta//36)*36 + 18.

Outputs under iterations/53/measure/:
    wave12-tinted.png    reference crop with wave-12 shapes tinted red
    wave12-zoom-<id>.png mask + image zoom of two sample shapes
    wave12-vertices.json canonical-frame vertex clusters + per-shape areas
"""
...
Image.fromarray(tinted[y0:y1, x0:x1]).save(OUT / "wave12-tinted.png")
...
for s in real[:2]:
    ...
    Image.fromarray(big).save(OUT / f"wave12-zoom-{sid}.png")
...
(OUT / "wave12-vertices.json").write_text(json.dumps({

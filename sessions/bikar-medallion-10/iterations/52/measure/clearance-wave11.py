#!/usr/bin/env python3
"""Iter-53 wave-11 clearance proof — rasterize the 20 snapped teardrops
(10 dart axes x {+, mirror}) in reference px space; count overlap px
against reference waves 1-10 (must be ~0) and wave-11 (on-target)."""
...
fit = json.loads((SESS / "iterations/52/measure/wave11-fit.json").read_text())
model = [(v["r"], v["rel"]) for v in fit["vertices"]]
...
for wnum in range(1, 23):
    ...
    ov = (stamp & wmask).sum()
    tag = "ON-TARGET" if wnum == 11 else ("OK" if ov == 0 else "OVERLAP!")

#!/usr/bin/env python3
"""Iter-53 wave-11 measurement — segment the reference exactly as plan-waves
does, pull the wave-11 shape masks, and extract canonical-frame vertex
...
the 36-deg STAR-TIP axis (0 mod 36), reproduces the 20 deep-navy wave-11
tiles? Plan thetas (80.0/100.1 about 72/108, 64.3/115.7, 44.3/135.7) say
mirror PAIRS straddling the star-tip axes at rel ~ +-8 deg — wave-7/10
pair topology but on the star-tip axis family, so th_axis here is
round(theta/36)*36 and the model is one free pair member.

Outputs under iterations/52/measure/:
    wave11-tinted.png    reference crop with wave-11 shapes tinted red
    wave11-zoom-<id>.png mask + image zoom of two sample shapes
    wave11-vertices.json canonical-frame vertex clusters + per-shape areas
"""
...
Image.fromarray(tinted[y0:y1, x0:x1]).save(OUT / "wave11-tinted.png")
...
for s in real[:2]:
    ...
    Image.fromarray(big).save(OUT / f"wave11-zoom-{sid}.png")
...
(OUT / "wave11-vertices.json").write_text(json.dumps({

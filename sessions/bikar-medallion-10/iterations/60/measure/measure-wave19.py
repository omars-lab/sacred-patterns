"""Iter-60 wave-19 measurement — segment the reference exactly as plan-waves
does, pull the wave-19 shape masks, and extract canonical-frame vertex
...
to the 36-deg STAR-TIP axis (0 mod 36), reproduces the 20 small
deep_navy wave-19 tiles (~84 u^2, r_frac 0.764-0.773)? Plan thetas sit
at rel +-7.1..8.0 about the star-tip axes — ONE MIRROR PAIR per axis
(wave-16 off-axis topology at the outer edge). Small tile: probe both
grids per protocol — the smallness driver alone predicts escalation
to g=0.25 (w15/w16 precedent).

Outputs under iterations/60/measure/:
    wave19-tinted.png    reference crop with wave-19 shapes tinted red
    wave19-zoom-<id>.png mask + image zoom of four sample shapes
    wave19-vertices.json canonical-frame vertex clusters + per-shape areas
"""
...
w18 = next(w for w in plan["waves"] if w["wave"] == 19)
...
Image.fromarray(tinted[y0:y1, x0:x1]).save(OUT / "wave19-tinted.png")
...
for s in sorted(real, key=lambda s: abs((s['theta_deg'] + 18) % 36 - 18))[:4]:
    ...
    Image.fromarray(big).save(OUT / f"wave19-zoom-{sid}.png")
...
(OUT / "wave19-vertices.json").write_text(json.dumps({

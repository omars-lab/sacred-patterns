...
**Migrating between versions:** this doc is its own change history — there is no separate migration guide. The **Status:** line above is the amendment log (every bump, dated, with the commits that shipped it); each `## Attributes — vN.N` section below is that version's walkthrough — what changed, the producer/consumer/witness evidence, and the fallback when the attribute is absent. To see what a version bump means for a mirror or a consumer, read its `## Attributes — vN.N` section.
...
| `data-orb-base-face` | **ACCEPTED 2026-08-02** (PROPOSED 2026-07-25, M3). Index of the base-polyhedron face this projected pattern face was lifted through — the 3D provenance needed to regroup per-tile faces after projection destroys 2D adjacency. Emitted on face `<path>` by `buildViewFaceAttrs`; in gt.json as `params.orb_base_face` via `buildOrbViewShape` in `gt-emitter.ts`. **Narrowed (v1.6 ACCEPTED 2026-09-09, D-052 — see §v1.6):** this index is a *true base-polyhedron face only*, present on inscribed cell views and the bare base solid and **absent** on wheelfield cell views, which carry `data-orb-unit` for their generative-unit provenance instead. `buildViewFaceAttrs` / `buildOrbViewShape` now emit it only for the face kind (`scene.baseIndexKind === 'face'`, bikar#169 `09390b8`, GT_SCHEMA_VERSION 1.28 → 1.29); before the narrowing `buildOrbViewShape` emitted it unconditionally (`orb_base_face: polygon.baseFaceIndex`). | `Contour.orb_base_face: int \| None` (SCHEMA 1.22), same read site. | qiyas falls back to ungrouped per-face scoring. Re-deriving the grouping from the projection is precisely the re-derivation Tenet 23 exists to prevent — two faces from different base tiles can land adjacent and indistinguishable. |
...
before — the producer, the consumer and the witnesses existed and merged. They now do, and Omar
approved the flip (2026-09-09):

- **Producer — bikar:** #169 `09390b8` split the wheelfield unit from the base-face claim —
  `buildViewFaceAttrs` stamps `data-orb-base-face` only when `scene.baseIndexKind === 'face'`
  and `data-orb-unit` when `'unit'` (SVG `030e2d3`) — and threaded the kind through
  `buildOrbViewShape` so gt.json emits `orb_base_face` only for the face kind,
  `GT_SCHEMA_VERSION` bumped 1.28 → 1.29. #170 `c2fa085` then made the *second* stamp layer
  honest: the breakdown's bare base solid renders through the scaffold underlay
  (`scaffoldElements`), which had hardcoded `'face'`, so both the plan
  (`OrbViewPlan.baseIndexKind`, derived per family in the CLI) and the render path had to
  branch, or a wheelfield's drawn cells said `unit` under a scaffold still saying `base-face`.
- **Consumer — 3d-models:** #170 `19e885d` landed the T9 gate
  (`.claude/gates/timelapse_gate.py`) **branched on orb family** — the manifest's own `base`
  block is the discriminator (`base.faces == 0` ⟺ a faceless wheelfield): a lifted orb keeps
  the base-face presence / range / subset check, a wheelfield asks the same three questions of
  `data-orb-unit` and treats **any** `data-orb-base-face` as a finding. `--self-test` grew a
  wheelfield fixture and three by-design mutators (a claimed face, blanked units, an orphaned
  unit) that each fire. Closes #49.
- **Witness — qiyas:** #29 `8362ce6`. qiyas is **n/a** for the generative unit (see the
  `data-orb-unit` row), so its obligation is the *negative* one — the reader must never misread
  a unit as a base face — pinned by `test_data_orb_unit_is_inert_to_the_base_face_reader` (a
  `data-orb-unit` element with no `data-orb-base-face` yields `Contour.orb_base_face is None`).
  It needed **no** `validate_dsl_contract.py` change and **no** gt re-record: qiyas's GT
  requirement is a `>= 1.24` lower bound the producer's 1.29 satisfies, and a wheelfield cell
  simply omitting `data-orb-base-face` reads as absent, not as a violation.

**Why this amendment goes into the canonical doc first, as a draft.** The cascade that
implements it (below) was originally planned canonical-*last* on the premise that the mirrors
are held byte-identical by an automated gate (bikar "hook 41" / 3d-models `schema_mirror.py`).
That premise is **false for this prose contract**: `schema_mirror.py` and its bikar counterpart
compare the vendored **JSON schemas** (`packages/qiyas-schema/schemas/` ↔
`src/qiyas/contract/schemas/`), *not* `dsl-metadata-contract.md`. Nothing in any repo compares a
prose mirror to this canonical doc — all three carry a hand-maintained "Mirrored canonical
version" header. With no equality gate forcing the order, the chosen order (owner-directed
2026-09-09) is **canonical-first-as-draft**: publish the authoritative *meaning* here so the
producers implement against a written spec, and flip it to ACCEPTED last. The bikar and qiyas
mirrors carried these rows as PROPOSED, pointing here, until this flip; with v1.6 ACCEPTED each
re-vendors its "Mirrored canonical version" header to v1.6.

**The defect, in one line.** `data-orb-base-face` carries two meanings under one name: a true
base-polyhedron face for inscribed orbs, and a *generative unit* (wheel `0..19` / filler
`20..31`) for wheelfield orbs. The v1.4 row documented it honestly-by-doc but the name lies for
wheelfield — an index past the base face count is a unit, not a face. 3d-models' T9 gate
surfaced it: a wheelfield breakdown labelled `base face(s) 15, 16, 20, 25, 31 on a 12-face
solid`.

**The fix (the robust one, not the cheap fork).** One name, one meaning:
`data-orb-base-face` / `orb_base_face` = a true base-polyhedron face only — present on inscribed
cell views and the bare base solid, **absent** on wheelfield cell views (and, already, on ribbon
views). The wheelfield generative unit is **preserved** under its own honest name `data-orb-unit`
(SVG), because a wheelfield cell has exactly one real generative unit and dropping it would
discard a real capability. gt.json **omits** `orb_base_face` for wheelfield shapes exactly as
`buildRibbonShape` already omits it for strands.

**Producer / consumer, as shipped.** Producer — bikar (#169 / #170, above): the SVG half keys
on `scene.baseIndexKind` and the gt half omits `orb_base_face` for wheelfield shapes,
`GT_SCHEMA_VERSION` 1.28 → 1.29. Consumer — 3d-models: the T9 check in
`.claude/gates/timelapse_gate.py` **branches on orb family** rather than checking
`data-orb-base-face` universally — a wheelfield scaffold now stamps `data-orb-unit`, so the gate
reads the unit label instead of treating its absence as trivially satisfied. (The earlier
"universal, no surface-branching" plan was dropped once the scaffold itself was made honest;
`fixing the producer beat forking the reader` — see
`3d-models/docs/issues/base-face-honesty-scaffold-pivot.md`.) qiyas is **n/a** — it has no use
for the generative-unit index, so `data-orb-unit` is not a qiyas-round-tripped attribute (like
`data-authored-region`); it needed **no** `validate_dsl_contract.py` change, because a wheelfield
cell that simply omits `data-orb-base-face` already reads as absent, and its only obligation —
never misreading a unit as a base face — is pinned by the inertness witness above.

| Attribute | Producer (DSL → emit) | Consumer | Fallback when absent |
|---|---|---|---|
| `data-orb-base-face` **narrowed to a true base face** | **ACCEPTED 2026-09-09** (PROPOSED 2026-09-09, D-052). A true base-polyhedron face index only — present on inscribed cell views and the bare base solid, **absent** on wheelfield cell views and ribbon views. bikar: `buildViewFaceAttrs` / `buildOrbViewShape` emit it only for the face kind (`scene.baseIndexKind === 'face'`; the wheelfield cell kind emits `data-orb-unit` instead), #169 `09390b8`. Supersedes the v1.4 row's "generative unit" reading for wheelfield; the v1.4 row's inscribed-orb behaviour is unchanged. | `Contour.orb_base_face: int \| None` (qiyas SCHEMA 1.22, unchanged) for the faces that carry it; 3d-models T9 asserts every emitted index is in `[0, base.faces)`. | Absent on a wheelfield cell view is now the **specified** state, not a drop; qiyas needs no exemption — a wheelfield cell that omits it reads as absent (`Contour.orb_base_face is None`), pinned by the inertness witness (qiyas#29 `8362ce6`). A consumer that synthesises a face index for a wheelfield cell is inventing provenance. |
| `data-orb-unit` | **ACCEPTED 2026-09-09** (PROPOSED 2026-09-09, D-052). The generative-unit index a wheelfield cell was drawn from — wheel `0..19` / filler `20..31`, carried from the field solve rather than re-derived. Emitted on wheelfield cell `<path>` by `buildViewFaceAttrs` / `renderOrbViewSVG` when `scene.baseIndexKind === 'unit'` (bikar#169 `09390b8`, SVG `030e2d3`). This is the honest name for what `data-orb-base-face` used to overload; it is **not** a base face and is **not** bounded by `base.faces`. Present only on wheelfield cell views. | **3d-models' T9 / future per-unit grouping — not qiyas.** Like `data-authored-region`, a producer-side provenance fact whose named consumer is not qiyas; qiyas has no use for a generative-unit index and does not round-trip it, so it gets **no `Contour` field and no round-trip**; its only qiyas witness is the *negative* one — `test_data_orb_unit_is_inert_to_the_base_face_reader` (qiyas#29 `8362ce6`), that a `data-orb-unit` element is never read as a base face — and it has **no row in the qiyas mirror's "Currently covered" table**. | A consumer without it cannot tell which wheel/filler a wheelfield cell came from; T9 falls back to no per-unit grouping. Never synthesised from geometry — two cells from different units can be congruent. |
| gt.json `orb_base_face` **omitted for wheelfield** | **ACCEPTED 2026-09-09** (PROPOSED 2026-09-09, D-052). `buildOrbViewShape` omits the `params.orb_base_face` key for wheelfield cell shapes — **not** `-1` and **not** `null` — exactly as `buildRibbonShape` already omits it for strands. Emitted only for face-kind shapes (bikar#169 `09390b8`, GT_SCHEMA_VERSION 1.28 → 1.29). Follows the omission discipline `data-shape-id` established. | qiyas's base-face round-trip stays **quiet** when neither the SVG nor the gt declares a base face (the `gt_consistency_gate` / `orb-validate` path); the score is unaffected — `orb_base_face` never fed a composite. | Absence is the specified state; there is no fallback to describe. |

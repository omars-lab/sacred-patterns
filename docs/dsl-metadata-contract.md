...
| `data-orb-base-face` | **ACCEPTED 2026-08-02** (PROPOSED 2026-07-25, M3). Index of the base-polyhedron face this projected pattern face was lifted through — the 3D provenance needed to regroup per-tile faces after projection destroys 2D adjacency. Emitted on face `<path>` by `buildViewFaceAttrs`; in gt.json as `params.orb_base_face` via `buildOrbViewShape` in `gt-emitter.ts`. **Proposed narrowing (v1.6, D-052, draft — see §v1.6):** this index is a *true base-polyhedron face only*, present on inscribed cell views and the bare base solid and **absent** on wheelfield cell views, which carry `data-orb-unit` for their generative-unit provenance instead. Today `buildOrbViewShape` emits it unconditionally (`gt-emitter.ts` `buildOrbViewShape`, `orb_base_face: polygon.baseFaceIndex`); the narrowing makes it conditional on the face kind. | `Contour.orb_base_face: int \| None` (SCHEMA 1.22), same read site. | qiyas falls back to ungrouped per-face scoring. Re-deriving the grouping from the projection is precisely the re-derivation Tenet 23 exists to prevent — two faces from different base tiles can land adjacent and indistinguishable. |
...
## Attributes — v1.6 PROPOSED (base-face honesty: `data-orb-base-face` narrowed, `data-orb-unit` added)

**Status (2026-09-09):** **PROPOSED**, not ACCEPTED. The substance is owner-directed
(`3d-models/docs/decisions-log.md` D-052, 2026-09-02):
`data-orb-base-face` / gt.json `orb_base_face` must mean **exactly one thing everywhere — the
index of the base-polyhedron face a projected element was lifted through**. But this doc marks
the change PROPOSED rather than ACCEPTED because, on the same footing as v1.4 and v1.5, owner
approval is granted *after — not before* — the producer, the consumer and the witnesses exist,
and at the time this row is written none of them do: the bikar producer split
(`data-orb-unit` emit + the gt-emitter omission) and the 3d-models T9 consumer are still to
land. The flip to **ACCEPTED** is a small closing amendment once bikar, qiyas and 3d-models
have all merged and their witnesses are green.
...
mirrors carry these rows as PROPOSED (pointing here) until that flip.
...
**Producer / consumer that must exist before this is ACCEPTED.**
Producer — bikar: the SVG half is built on branch `feat/base-face-honest-attr`
(`renderOrbViewSVG` keyed on a new scene `baseIndexKind`, emitting `data-orb-unit` vs
`data-orb-base-face`); the gt half threads the scene kind through `buildOrbViewShape` (today
`gt-emitter.ts` emits `orb_base_face: polygon.baseFaceIndex` unconditionally at line ~2165) so it
is emitted only for the face kind, `GT_SCHEMA_VERSION` bumped 1.28 → 1.29.
Consumer — 3d-models: the T9 base-face check in `.claude/gates/timelapse_gate.py` (a true face is
always in `[0, base.faces)`; wheelfield cells carry none, so the subset check is trivially
satisfied — no surface-branching). qiyas is **n/a** — it has no use for the generative-unit
index, so `data-orb-unit` is not a qiyas-round-tripped attribute (like `data-authored-region`);
qiyas's only obligation is to exempt wheelfield cell views from the strict `data-orb-base-face`
presence requirement via `validate_dsl_contract.py`'s `allow_absent`, the mechanism that already
exempts ribbon views.

| Attribute | Producer (DSL → emit) | Consumer | Fallback when absent |
|---|---|---|---|
| `data-orb-base-face` **narrowed to a true base face** | **PROPOSED (v1.6, 2026-09-09, D-052).** A true base-polyhedron face index only — present on inscribed cell views and the bare base solid, **absent** on wheelfield cell views and ribbon views. bikar: `buildViewFaceAttrs` / `buildOrbViewShape` emit it only for the face kind (the wheelfield cell kind emits `data-orb-unit` instead). Supersedes the v1.4 row's "generative unit" reading for wheelfield; the v1.4 row's inscribed-orb behaviour is unchanged. | `Contour.orb_base_face: int \| None` (qiyas SCHEMA 1.22, unchanged) for the faces that carry it; 3d-models T9 asserts every emitted index is in `[0, base.faces)`. | Absent on a wheelfield cell view is now the **specified** state, not a drop; qiyas exempts it via `allow_absent` (as ribbon views). A consumer that synthesises a face index for a wheelfield cell is inventing provenance. |
| `data-orb-unit` | **PROPOSED (v1.6, 2026-09-09, D-052).** The generative-unit index a wheelfield cell was drawn from — wheel `0..19` / filler `20..31`, carried from the field solve rather than re-derived. Emitted on wheelfield cell `<path>` by `renderOrbViewSVG` (branch `feat/base-face-honest-attr`). This is the honest name for what `data-orb-base-face` used to overload; it is **not** a base face and is **not** bounded by `base.faces`. Present only on wheelfield cell views. | **3d-models' T9 / future per-unit grouping — not qiyas.** Like `data-authored-region`, a producer-side provenance fact whose named consumer is not qiyas; qiyas has no use for a generative-unit index and does not round-trip it, so it gets **no `Contour` field, no qiyas witness, and no row in the qiyas mirror's "Currently covered" table**. | A consumer without it cannot tell which wheel/filler a wheelfield cell came from; T9 falls back to no per-unit grouping. Never synthesised from geometry — two cells from different units can be congruent. |
| gt.json `orb_base_face` **omitted for wheelfield** | **PROPOSED (v1.6, 2026-09-09, D-052).** `buildOrbViewShape` omits the `params.orb_base_face` key for wheelfield cell shapes — **not** `-1` and **not** `null` — exactly as `buildRibbonShape` already omits it for strands. Emitted only for face-kind shapes. Follows the omission discipline `data-shape-id` established. | qiyas's base-face round-trip stays **quiet** when neither the SVG nor the gt declares a base face (the `gt_consistency_gate` / `orb-validate` path); the score is unaffected — `orb_base_face` never fed a composite. | Absence is the specified state; there is no fallback to describe. |

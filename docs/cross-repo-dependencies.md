...
| stored session renders, crops + overlays (`~/Dropbox/Data/sacred-patterns/<name>/`) | gt.json consumers; y-asymmetric calibrations | **bikar #55 / #57** (decision `bikar/docs/decisions/2026-08-13-render-orientation.md`) | bikar constrains both | open — shipped bikar-side 2026-08-13, **not yet pushed** | **bikar's renders were upside down and are now the right way up.** Its DSL works in the ordinary maths plane (+y up) and wrote those coordinates straight into SVG (+y down), so every picture it ever drew was a vertical mirror of the file that produced it; fixed 2026-08-13 by reflecting once at the output boundary. 27 of the 110 committed patterns now render differently, and 8 of those are chiral — the new render is not any rotation of the old one. `.gt.json` follows the picture: pixel row 0 is the top, and an arc's `sweepCCW` is now a claim about pixels. **`GT_SCHEMA_VERSION` does not change**, so nothing announces the shift — any render, crop, overlay or pixel-diff sitting in a session directory from before this date was made in a differently-oriented world, and an iteration verdict resting on one is stale. Re-render before reading an old overlay. |

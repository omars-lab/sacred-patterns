...
The baseline gates Step 2 (Analyze Pattern) and is consumed by `arch-audit.py --baseline` every iteration.
...
- **`qiyas pixel-diff`** (Docker) — Pixel-based image diff. Renders both inputs to bitmaps, produces visual overlay, heatmap, and numeric similarity score. Owned by qiyas; called from the orchestrator via `docker run ghcr.io/naqshcoffee/qiyas:dev pixel-diff ...` with `--rasterizer magick` for stroke fidelity. Pixel similarity is regression-detection only, NOT the convergence goal (see "Convergence Philosophy").
- **`arch-audit.py`** — Architectural convergence audit (A2-A6): N-fold symmetry, tile distribution, coverage, band integrity, baseline shape validation. Auto-detects pattern parameters from clip circles. The optional `--baseline` flag enables A6 against the user-verified expectations from interpret-pattern. Will move to `qiyas svg-audit` in Phase 3.
